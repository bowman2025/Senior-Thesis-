...
# Cleaned and organized for public sharing
...
data <- read.csv("/mnt/data/Merged_Data_Cleaned.csv")
...
install.packages("nnet")  # Only if you haven't already
library(nnet)
...
# Assuming 'Favorability' data is in columns like 'Teacher.favorability', 'Veteran.favorability', 'Mechanic.favorability'
...
# Assuming your dataset is 'subset_data' and the favorability variables are named like 'Teacher.favorability', 
...
populist_vals <- data.frame(selected_all_populist = c(1, 0),  # Populist vs non-populist framing
                            Party.registration.numeric = 1,  # Assuming Republican Party for example
                            age = mean(subset_data$age, na.rm = TRUE),
                            education.level.numeric = mean(subset_data$education.level.numeric, na.rm = TRUE),
                            sex = "Male",  # Assume a reference category for Sex
...
populist_vals <- data.frame(
  ...
  Party.registration.numeric = 1,  # Assume Republican Party for example
  age = mean(subset_data$age, na.rm = TRUE),
  education.level.numeric = mean(subset_data$education.level.numeric, na.rm = TRUE),
  sex = "Male",  # Assume a reference category for Sex
  income.numeric = mean(subset_data$income.numeric, na.rm = TRUE)
...
ggplot(candidate_party_selection, aes(x = Candidate, y = Percentage, fill = Party.registration)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
# Check the count of NA values for Veteran selection
sum(is.na(subset_data$Veteran))

# Check the number of selections for the Veteran candidate by different groups
...
ggplot(candidate_rep_strength, aes(x = factor(Republican.Strength), y = Candidate, fill = Selections)) +
  ...
  theme_minimal() +
  scale_x_discrete(labels = c("Not Very Strong", "Strong"))
# Check if there are any NAs in the Selection column
...
# Check the dataset to confirm that NA and zero values are removed
...
# Check the first few rows of the interaction data to ensure it's set up correctly
...
library(dplyr)
 ...
 # Check the new binary variables
 head(Merged_Data)
 library(dplyr)
 library(ggplot2)
...
theme_minimal() +
theme(axis.text.x = element_text(angle = 45, hjust = 1)
 # Assuming split-ticket voting data is in a variable named `split_ticket_voting`
 library(dplyr)
 ...
 ggplot(Merged_Data %>% filter(!is.na(Split.ticket.voting), !is.na(Party.registration)),
   ...
   theme_minimal()
 # Assuming Q6 contains issue importance data as factors
 ...
 head(Merged_Data$split_issues)
 # Check if 'Drug addiction' is listed in the Important.issues column
...
Merged_Data_Cleaned <- Merged_Data_Cleaned %>%
 ...
 library(car)
 vif(model_rural_party)  # Check the VIF values
...
# Check the new levels to confirm the change
...
ggplot(coefficients_df, aes(x = Estimate, y = Predictor, xmin = Estimate - `Std. Error`, xmax = Estimate + `Std. Error`)) +
  ...
  theme(axis.text.y = element_text(size = 8, face = "bold", angle = 0, hjust = 1),  # Smaller font size for y-axis labels
        ...
        axis.title = element_text(size = 12)) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 15))  # Wrap y-axis labels for better readability
install.packages("tm")        # For text mining and processing
...
# Check Variance Inflation Factor (VIF) to address multicollinearity
library(car)
vif(model_improved)
...
# Check column names in coefficients_df
colnames(coefficients_df)

# Assuming 'Std. Error' is the correct column name for standard errors
ggplot(coefficients_df, aes(x = Estimate, y = reorder(Predictor, Estimate), xmin = Estimate - 1.96 * `Std. Error`, xmax = Estimate + 1.96 * `Std. Error`)) +
  geom_point() +
  geom_errorbarh(height = 0.2) +
  labs(title = "Coefficient Plot: Predictors of Economic Populism Favorability",
...
# Check unique values for each reason-related column
...
# Check levels or unique values of each covariate
summary(Merged_Data_Cleaned$sex)
summary(Merged_Data_Cleaned$Race_Recode)
summary(Merged_Data_Cleaned$Education_Group)
...
# Check unique values in each predictor
sapply(Merged_Data_Cleaned[, c("sex", "Race_Recode", "Education_Group", "income")], function(x) length(unique(na.omit(x))))
...
# Optional: Check the updated summary to confirm change
print(summary_df)
...
# Assuming your final summary dataframe is called summary_df and includes:
...
# Assume summary_df already exists with required columns
